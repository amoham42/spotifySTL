...
function init() {
    ...
    const ground = new Mesh(new PlaneGeometry(400, 400), new MeshPhongMaterial({color: 0x000000, depthWrite: false}));
    ground.position.x = 30;
    ground.rotation.x = -Math.PI / 2;
    ground.receiveShadow = true;
    scene.add(ground);

    const grid = new GridHelper(400, 50, 0xbbbbbb, 0xbbbbbb);
    grid.position.x = 30;
    ...
    objloader.load('keychain.obj', function (object) {
...
function spot (data) {
    ...
    for (let i = update; i < paths.length; i++) {
        ...
        const path = paths[i];
        const material = new MeshPhongMaterial({color: 0x1ED760});
        const shapes = SVGLoader.createShapes(path);

        for (let j = 0; j < shapes.length; j++) {

            const shape = shapes[j];
            const geometry = new ExtrudeGeometry(shape, {depth: 7, bevelEnabled: false});
